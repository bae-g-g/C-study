...
> 이전 학습: 이전 문서 없음  
> 다음 학습: [02. 컴파일러 과정](./02_컴파일러_과정.md)

<p align="center">
  <img src="./img/image1.png" alt="01. 빌드 과정 대표 이미지" width="720">
</p>

### 학습 목표

- 빌드 과정의 핵심 개념과 사용 목적을 설명할 수 있다.
...
1. C/C++
2. Compile
3. CPU에서 실행 가능한 파일(.exe) 생성(Compile)
4. Run
5. CPU에서 실행 가능한 파일(.exe) 실행(Run)
6. Compile +
...
| 작 동     | 소스 코드 ➝ 기계어                       | 기계어                         |
| 가상 머신 | 가상 머신 x                              |                                |
| 중간 코드 | 중간 코드 x: 소스 코드 ➝(Direct)➝ 기계어 |                                |
| 파 일     | .c / .cpp ➝ .exe                         | .exe                           |
...
- 따라서 Window OS와 Mac OS와 실행 결과 상이
5. Hard Disk에 할당
...
- 핵심: Step #1-A) 1-B) 1-C)는 "동시"에 "병행" 처리
...
---
<!-- LESSON-NAV:START -->
이전: 없음 · [단원 목차](./README.md) · [다음: 02. 컴파일러 과정](./02_컴파일러_과정.md)
<!-- LESSON-NAV:END -->

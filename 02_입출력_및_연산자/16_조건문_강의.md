...
> 이전 학습: [13. 컴퓨터 구조와 임시변수](./13_컴퓨터_구조와_임시변수.md)  
> 다음 학습: [01. 반복문 강의](../03_제어문/01_반복문_강의.md)

### 학습 목표

- 조건문 강의의 핵심 개념과 사용 목적을 설명할 수 있다.
...
2. if
3. Form 형식
4. Condition/Conditional Expression 조건/조건식 ①
5. Code Block 수행문 ②
6. Compiling/Running 실행 ③
...
조건/조건식 (else: 조건/조건식 없음) = True ➝ 해당 Code Block 실행 O
...
➝ 조건/조건식 2 해당 Code Block 실행 O ➝ 조건문 종면

- All 조건/조건식 = False ➝ 조건문 종료 (else가 있으면: else 해당 Code Block 실행 후 조건문 종료)
...
nB) else 있음 = Code Block n 실행 ➝ 조건문 종료
...
- 참고: if 조건문 형식에서 { } 다음에 다른 Code 작성 불가
- 이유: if 문 조건문 = 반드시 if로 시작 → 다음 if 가 나올때 까지 { } 중괄호 다음에 다른 Code 작성 X
...
- 해당 조건 = True
- ➝ 해당 Code Block 실행 ➝ break문 실행(break문 없음: 조건문 종료 X) ➝ 조건문 종료

#### 예시

- break문 있음
- case 변수에 음기화된값: ①의 조건 = False
...
- 참고: float: 작은 자료형(메모리소모 작음 ➝ 연산속도 증가)
- 참고: double: 큰 자료형(표현범위 큼 ➝ 정확도 증가)
...
| Compiling<br>Running | 해당 조건/조건식 = True<br>➝ 해당 Code Block 실행<br>➝ 조건문 종료 | 해당 조건 = True (break문 있음)<br>➝ 해당 Code Bl음ck 실행<b음>➝ Break문 실행<br>➝ 조건문 종료<br>해당 조건 = True (break문 없음)<br>➝ 해당 조건 + 아래 All Code Blocks 실행<br>➝ 조건문 종료 |
| Break                | 사용 X                                                             | 사용 O                                                                                                                                                                                        |
...
---
<!-- LESSON-NAV:START -->
[이전: 13. 컴퓨터 구조와 임시변수](./13_컴퓨터_구조와_임시변수.md) · [단원 목차](./README.md) · [다음: 01. 반복문 강의](../03_제어문/01_반복문_강의.md)
<!-- LESSON-NAV:END -->
```
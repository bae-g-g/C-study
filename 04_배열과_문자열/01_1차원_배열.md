...
> 단원: [04. 배열과 문자열](./README.md)  
> 이전 학습: [02. 분기문](../03_제어문/02_분기문.md)  
> 다음 학습: [02. 다차원 배열](./02_다차원_배열.md)

<p align="center">
  <img src="./img/J.png" alt="01. 1차원 배열 대표 이미지" width="720">
</p>

### 학습 목표

- 1차원 배열의 핵심 개념과 사용 목적을 설명할 수 있다.
...
- **클라스** (C언어에는 없음, C++, Python, Java에는 존재함)
- 구조체 + 함수(라이브러리 내 기본 함수 + 사용자 지정 함수 + 라이브러리 내 사용자 지정 함수)

#### 예시: 구조체 변수 human + Diet 함수(몸무게 0.5 kg 감소 함수 → 사용자 지정 함수)
...
- 문자: 배열크기 => 요소 갯수 +1
- 이유: `scanf %s` 문자열 마지막 \\0 = null 문자 자동 초기화
- 이유: `printf %s` \\0 = null 문자 전까지 출력
...
- 참고: 선언시 메모리박스(Memory Box) 크기(배열크기)만큼 자동 \\0 = null Set-Up X
...
- 이유: printf %s(문자 '열' 출력서식) 같은 정수/소수 '열' 출력서식 존재 X
...
- 참고: 배열선언시 \\0=null Set-Up 필요 O
- 이유: printf %s (\\0=null 문자 전까지 출력) 사용 O
- 참고: 배열크기 ≥ 초기화 요소갯수 + 1개(\\0= null 문자 메모리박스 공간)
- 참고: 배열크기 < 초기화 요소갯수
...
- 참고: 공백문자(엔터, 스페이스, 탭) 메모리박스(Memory Box) 문자인식 O
...
- 참고: 선언시 \\0 = null Set-Up 필요 X ∵ printf %s (\\0=null 문자 전까지 출력) 사용 X
- 참고: 배열크기 ≥ 초기화 요소갯수
...
- 참고: 문자열 마지막 \\0 = null 문자 생성 O
- 참고: 배열선언시 \\0 = null Set-Up 필요 X ∵ scanf %s 문자열 마지막 \\0 = null 문자 생성 O
- 참고: 배열크기 ≥ 초기화 변수갯수 + 1개(\\0 = null 문자 생성 공간)
- 참고: 배열크기 < 초기화 변수갯수
...
- 참고: 문자열 마지막 \\0 = null 문자 생성 X
- 참고: 배열선언시 \\0 =null Set-Up 필요 O ∵ printf %s (\\0 = null 문자 전까지 출력) 사용 O
- 참고: 배열크기 = 초기화 변수갯수 + 1개 이상 (\\0 = null 문자 메모리박스 공간)
- 참고: 공백문자(엔터, 스페이스) 문자인식 O
- 1) 공백문자 '포함' '버퍼' 입력/저장
...
- 참고: 배열선언시 \\0 = null Set-Up 필요 X ∵ printf %s (\\0 = null 문자 전까지 출력) 사용 X
- 참고: 배열크기 ≥ 초기화 변수갯수
...
- 이유: 같은 Code Line에서 초기화시(= 선언 '동시' 초기화시) 요소갯수로 배열크기 Can Define
...
- 따라서 배열 크기 = 11 → vscode에서 Mouse Overall시 char \[11\] 확인 가능 O
...
### C++/Python Language에 있는 자료형 (C 에는 없는 자료형)

- 따라서 불편한점 발생 → 함수 라이브러리(.h 파일) 지원
...
- 이유: 문자열 함수 \\0 = null 문자 전까지 인식/실행 O
...
| toupper, tolower                                | O string\_1                         | X                                                                                                  | 변수에 초기화시<br>변경 문자열(%s) | 반환값 → 문자 1개(%c)<br><br>변경 문자열(%s)<br>∴ 출력값 없음 (Segmentation Fault)<br>∴ 전체 초기화 필요 O<br>초기화 하지 않아도<br>변경문자 1개 출력가능 O(%c)<br>변경문자 1개 ASCII Code 변환정수 출력가능 O(%d) |
...
```c
배열 요소 변경 O → 배열 요소 초기화 X → 변수 = 함수(string_1); 초기화 필요 O

| printf(%s, string\_1);                                      | 변경전 문자열 출력(string\_1)                                                      |
| ----------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| string\_2 = tolower(string\_1);<br>printf("%s", string\_2); | <br>변경후 문자열 출력(string\_2)                                                  |
| printf(%s, tolower(string\_1));                             | 반환값(Return) = 변경후 문자1개(%c 지정)<br>출력값 없ㅇ므 (Segmentation fault)     |
...
---
<!-- LESSON-NAV:START -->
[이전: 02. 분기문](../03_제어문/02_분기문.md) · [단원 목차](./README.md) · [다음: 02. 다차원 배열](./02_다차원_배열.md)
<!-- LESSON-NAV:END -->

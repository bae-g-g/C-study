...
> 단원: [05. 함수](./README.md)  
> 이전 학습: [02. 다차원 배열](../04_배열과_문자열/02_다차원_배열.md)  
> 다음 학습: [02. 반환값이 있는 함수](./02_반환값이_있는_함수.md)

### 학습 목표

- 함수 강의의 핵심 개념과 사용 목적을 설명할 수 있다.
...
- } 함수 끝
- 참고: Function Header: 반환 자료형 ① + 함수이름 ② + (매개변수) ③
- 참고: Function Body: Code Blocks ④ return + 반환값(정수/소수/문자); ⑤
...
printf("Design Embodiment Programming_C");
return 0;
...
int plus_calculation (int X, int Y)
...
```c
// 사용자정의 함수
int plus_calculation (int X, int Y) ⓑ // X = number_1, Y = number_2,
...
printf("%d",  plus_calculation(number_1, number_2);

    return 0;
}

- 참고: 자료형: int, float, char, array 통일 O, int, float, char, array 혼합 O
- 메인 함수 지역변수 형태(자료형) ⓐ = 사용자정의 함수 매개변수 형태(자료형) ⓑ
- 참고: 갯수: 0개 이상 (0개, 1개, 2개, 3개 .......)
...
#### 예시: if, else if, else, while, for 문 내 or 밖
...
- 참고: int 메인 함수/사용자정의 함수에서 함수의 자료형이 int
...
- 참고: Compiling
1. 메인 함수내 사용자정의 함수 호출
2. 메인 함수내 사용자정의 함수에 해당하는 변수 초기화값을 사용자정의 함수의 매개변수에 초기화
3. 사용자정의 함수 Code Blocks 실행
4. 사용자정의 함수 반환값(Return값) 반환
5. 메인함수내 '사용자정의 함수명(변수)'를 '사용자정의 함수 반환값(Return값)'으로 대체
...
- 참고: 함수 선언 '동시' 정의
1. 사용자정의 함수: Function Header + Function Body
2. 메인 함수
- **a\_function\_4.c**
...
- 참고: 함수 선언 '후' 정의
1. 사용자정의 함수: Function Header
2. 메인 함수
3. 사용자정의 함수: Function Header + Function Body
- **a\_function\_5.c**
...
- 1 → 정상종료
- 0 → 비정상종료
- 이유: 사용자정의 함수는 개발자가 Code로 직접 작성 → Code에서 0 = False, 1 = True

- **void main vs int main**

### 과거: void main 함수 와 표준(Standard)

- 이유: 반환값(Return값) 없음 → 1 Byte라도 더 적은 Memory 활용 가능 O

### 현재: int main 함수 → 표준(Standard)

- 이유: void main 함수 반환값(Return값) 없음 → 프로그램 정상종료? 오류발생 But 그냥종료? 알수 X

```c
void main 함수 → 활용
```

- 이유: 다른 Codes와의 호환성을 위해

- 참고: void main 함수 사용권장 X

* * *

---
<!-- LESSON-NAV:START -->
[이전: 02. 다차원 배열](../04_배열과_문자열/02_다차원_배열.md) · [단원 목차](./README.md) · [다음: 02. 반환값이 있는 함수](./02_반환값이_있는_함수.md)
<!-- LESSON-NAV:END -->

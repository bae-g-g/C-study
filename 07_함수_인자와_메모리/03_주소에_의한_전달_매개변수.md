...
> 이전 학습: [02. 값에 의한 전달](./02_값에_의한_전달.md)  
> 다음 학습: [04. 주소에 의한 전달 반환](./04_주소에_의한_전달_반환.md)

<p align="center">
  <img src="./img/CBA1.png" alt="03. 주소에 의한 전달 매개변수 대표 이미지" width="720">
</p>

### 학습 목표

- 주소에 의한 전달 매개변수의 핵심 개념과 사용 목적을 설명할 수 있다.
...
1. Call by Address
2. → Parameter = Pointer Variable
3. Significance 의의
4. 1) (n) of External Variables ≥ 2 외부 변수 갯수 ≥ 2
5. 1)A (n) of External Variables = 1 외부 변수 갯수 = 1
6. 1)B (n) of External Variables ≥ 2 외부 함수 변수 갯수 ≥ 2
...
- 외부에 정의된 함수(메인 함수 포함)의 "2개 이상의 변수값"을 "1개"의 사용자정의 함수에서 "한번에" 초기화/변경
...
- e.g.) X ➝(붕괴 O)➝ &X\[0\]
- 따라서 배열을 매개변수 Parameter로 사용하면 포인터변수로 사용됨
- 따라서 배열을 매개변수 Parameter로 사용하려면 인수를 포인터변수로 사용해야함
...
- 참고: Global Variable 전역 변수는 함수를 사용하여 변경하기보다 직접 변경 용이
...
- 핵심: 사용자정의 함수 사용 이유로 인해, 사용자정의 함수를 사용해야함

- 핵심: 사용자정의 함수 종료후에도 외부에 정의된 함수의 변수값 초기화/변경할 경우

- 외부에 정의된 함수의 "2개 이상의 변수값"을 초기화/변경할 경우
- 이유: 사용자정의 함수의 "Return 갯수는 1개" ➝ Return 사용 X

- e.g.) int function ( ){ } ➝ int 값 Return, void function ( ){ } ➝ Return 값 없음
- 따라서 "2개 이상의 변수값"을 초기화/변경할 경우 포인터변수를 사용할수 밖에 없음
- 참고: 대부분 Void 사용자정의 함수 사용 ➝ Return 사용 필요 X
...
- 핵심: 아래의 사용자정의 함수 사용 이유로 인해, 사용자정의 함수를 사용해야함
...
- 따라서 "2개 이상의 변수값"을 초기화/변경할 경우 포인터변수를 사용할수 밖에 없음
- 참고: 대부분 Void 사용자정의 함수 사용 ➝ Return 사용 필요 X
- 때때로 Bool 사용자정의 함수 사용하여 함수 실행 동작 여부 Check 값
- 하지만 Code 작성 의도에 따라 int, float, char ... 사용자정의 함수 사용하기도 함
...
- 따라서 scanf %s 정상 출력을 위한 방법 1: 마지막 문자에 Null('\\0') 수동 입력
- 따라서 scanf %s 정상 출력을 위한 방법 2: 문자열 초기화를 위한 배열 선언시 Null('\\0') Set-up
...
- 참고: X 문자열 Length > Y 문자열 복사할 Length

- 예: X = ABCDEFG, Y = abcdefg, n = 3

abcDEFG

- 참고: X 문자열 Length < Y 문자열 복사할 Length

- 예: #1) X 배열 전체 메모리박스 크기 < Y 문자열 복사할 Length
...
- 예: X= ABCDEFG, Y = abc, n = 5 // Y = abc\\0\\0
...
- 예: #1) X 배열 전체 메모리박스 크기 < Y 문자열 Length
...
---
<!-- LESSON-NAV:START -->
[이전: 02. 값에 의한 전달](./02_값에_의한_전달.md) · [단원 목차](./README.md) · [다음: 04. 주소에 의한 전달 반환](./04_주소에_의한_전달_반환.md)
<!-- LESSON-NAV:END -->

...
> 이전 학습: [03. 주소에 의한 전달 매개변수](./03_주소에_의한_전달_매개변수.md)  
> 다음 학습: [05. 주소에 의한 전달 일반](./05_주소에_의한_전달_일반.md)

<p align="center">
  <img src="./img/CBA1.png" alt="04. 주소에 의한 전달 반환 대표 이미지" width="720">
</p>

### 학습 목표

- 주소에 의한 전달 반환의 핵심 개념과 사용 목적을 설명할 수 있다.
...
1. A 배열(배열이 붕괴된 포인터변수) 반환 "조차" 가능 X

- 예: return X; // int X\[3\]; : X ➝(붕괴 O)➝ &X\[0\]

➝ 0 반환

- 이유: Compiling Error #1: 배열 재초기화 불가능
...
- 이유: Compiling Error #2: C언어 없는 기능 설정

배열 크기가 너무 크면 함수에서 처리시 너무 비효율적이므로 없는 기능 설정

1. B 포인터변수 반환"은" 가능 O

- 예: return X\_ptr; // int X\[3\]; : int \*X\_ptr = &X\[0\];

2. 사용 범위가 지역 변수 (함수 내)

3. Life Cycle이 사용자정의 함수 종료후 포인터변수 Memory "해제"
...
- 이유: Stack Sector 메모리 "할당" 원리(Stack 쌓임) 특성

- 따라서 사용자정의 함수 Return (반환)한 포인터변수(주소값)으로 접근한 메모리 공간

≠ 배열 메모리 공간

실제 의도한 초기화값 (e.g. 배열 요소 값)

≠ 실제 배열 메모제 공간에 초기화된 값 (e.g. 사용자정의 함수호출 아래 i 값, printf 주소값)
...
3. Life Cycle이 사용자정의 함수 종료후 포인터변수 Memory "유지"
...
- 참고: 큰변수(e.g. 배열, 구조체, 클라스) + static 포인터변수 사용 의의
...
- 참고: 큰변수(e.g. 배열, 구조체, 클라스) + 동적 메모리 할당 포인터변수 사용 의의
...
---
<!-- LESSON-NAV:START -->
[이전: 03. 주소에 의한 전달 매개변수](./03_주소에_의한_전달_매개변수.md) · [단원 목차](./README.md) · [다음: 05. 주소에 의한 전달 일반](./05_주소에_의한_전달_일반.md)
<!-- LESSON-NAV:END -->

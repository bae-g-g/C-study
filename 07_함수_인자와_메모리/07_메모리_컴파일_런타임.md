...
> 이전 학습: [06. 참조에 의한 전달](./06_참조에_의한_전달.md)  
> 다음 학습: [08. 메모리 구조와 관리](./08_메모리_구조와_관리.md)

<p align="center">
  <img src="./img/mcr.png" alt="07. 메모리 컴파일 런타임 대표 이미지" width="720">
</p>

### 학습 목표

- 메모리 컴파일 런타임의 핵심 개념과 사용 목적을 설명할 수 있다.
- 예제 코드를 읽고 실행 흐름과 결과를 단계별로 추적할 수 있다.
- 이전 단원에서 배운 개념과 다음 단원에서 이어질 내용을 연결할 수 있다.

### 수업 흐름

1. Phase #1) Preprocessing 전처리
2. Phase #2) Compile 컴파일
3. Phase #3) Assemble 어셈블
4. Phase #4) Link 링크
5. 실행
6. Understanding
...
- 핵심: 아래 Step #1-A) 1-B) 1-C)는 "동시"에 처리
...
1. Terminal Window 터미널 창에서 Folder Path Exploration 폴더 경로 탐색
- cd ./ tab

2. gcc \-o 생성할 파일명 file1.c file2.c file3.c .... 을 입력 (파일 및 옵션은 띄어쓰기 구분)
...
- 참고: \[CH08A\] Variable (Lecture) 참조
- 참고: \[CH09A\] Pointer (Lecture) > Function > Memory (Management + Structure + Operation) 참조
...
#### **Understanding #2) 배열 크기 지정 ➝ 지역 변수 가능 O**

#### **Understanding #3) 배열 크기 지정➝ 매개 변수 가능 O**

1. 지역 변수 가능 O

2. 매개 변수 가능 O (매개 변수 = register 변수: CPU Register에 초기화 ➝ CPU Register에서 처리)
- 이유: - 배열 ➝(붕괴)➝ 포인터 (컴파일 2단계)

2. -ⓐ int X\[3\] or int X\[10\] 경우 포인터로 붕괴 처리하여 어떤 배열 Index 에서도 주소값으로 접근 O
- ➝ 어셈블리어 ( Image 오른쪽) Red Coloured Texts (QWORD PTR, DWORD PTR) 참조
![](https://t9003081320.p.clickup-attachments.com/t9003081320/de66b2a1-301b-44a3-ad05-64f32477dfcf/image%20(2).png)

2. -ⓑ 붕괴처리 예외 2 Cases (sizeof, &)는 소스 코드( Image 왼쪽)에서 배열로 처리 후
- 컴파일 2단계에서 "배열로 처리하는 형식"으로 붕괴 처리
- ➝ 어셈블리어 (Image 오른쪽) 19번째줄, 25번째줄 포인터 연산 단위 20, 40 참조
...
#### **Understanding #4) 배열 크기 없음 ➝ 지역 변수 가능 X**

- 지역 변수는 Run Time 시 할당(Memroy RAM - STACK)되는데도 불구하고, Complie Error 발생

- 이유: ### Run Time 시 할당되지만, Complie 과정에서 문법 검사 통과 X
...
int main ()
{
   int X[0];   // 배열 크기 지정 + 지역 변수 가능 O 
   // int Y[];    // 배열 크기 없음 + 지역 변수 가능 X
   return 0;
}
```
...
- 이유: - Compile 시 각각 4 Bytes, 4 Bytes, 1 Byte의 정확한 Memory 할당 크기 파악 가능 O
...
- 따라서 Run Time에 영향 없도록, 사전 Compile Error 발생 하도록 Set-Up

#### **Understanding #5) 배열 크기 없음 ➝ 매개 변수 가능 O**

1. 문법 정의: 인수 ➝(초기화)➝ 매개 변수: 초기화 시 인수"값만" 매개 변수에 전달
...
3. 문법 정의: 배열 매개 변수 ➝(붕괴)➝ 포인터 변수 (주소값)

- 따라서 배열 매개 변수 = 포인터 변수 (= 주소값, ≠ 배열 전체 요소값)으로 처리
- 따라서 배열 매개 변수 ➝ 배열 크기와 무관 O

**compile\_run\_5.c**
- 배열 크기 지정 ➝ 지역 변수 가능 O + 매개 변수 가능 O

배열 크기 없음 ➝ 지역 변수 가능 X + 매개 변수 가능 O
...
---
<!-- LESSON-NAV:START -->
[이전: 06. 참조에 의한 전달](./06_참조에_의한_전달.md) · [단원 목차](./README.md) · [다음: 08. 메모리 구조와 관리](./08_메모리_구조와_관리.md)
<!-- LESSON-NAV:END -->

...
> 이전 학습: [07. 메모리 컴파일 런타임](./07_메모리_컴파일_런타임.md)  
> 다음 학습: [09. 배열 반환](./09_배열_반환.md)

<p align="center">
  <img src="./img/mms3.png" alt="08. 메모리 구조와 관리 대표 이미지" width="720">
</p>

### 학습 목표

- 메모리 구조와 관리의 핵심 개념과 사용 목적을 설명할 수 있다.
...
2. Concept 개념
3. Purpose 목적
4. Memory Structure + Operation 메모리 구조 + 동작
5. Memory (RAM)
6. CODE (TEXT) & DATA Sector
...
- Computer Memory(RAM)는 용량 제한이 있음
...
### ↓ free 사용자지정 함수 사용하여 Memory "해재" 필요

- 참고: auto, static 변수는 ("할당" ➝ "해제") 반복
...
- 참고: ROM: Read Only Memory (Persist)
- 참고: RAM: Random Access Memory (Volatile)
- 핵심: CODE, CONST, crt0 = Run Time시 변경불가 X ➝ RAM의 일정부분을 ROM으로 설정

* * *


아래 영역 용어 ➝ 메모리 Memory = RAM
...
- 따라서 Compile Error 발생 X
- 참고: 초기화값 = 사실상 쓰레기값 초기화 되어 있음
- ➋ Compile후(.exe 파일 생성후) Run Time시(.exe 파일 실행시)
- RAM에 메모리 "크기만" 전달(= 메모리크기"만" RAM에 "할당")
- 상태: "전달"된 X라는 이름의 메모리 4 Bytes ➝ "할당" ➝ 초기화값 0 "실행"
- 참고: static 변수의 특성: 초기화하지 않으면 0으로 자동 초기화 실행이 Run Time시 실행됨

배열 변수

배열 메모리 "할당" = (배열 요소 갯수 \* 배열 요소 자료형 크기) "할당"
- 메인 함수 작성시 아래 내용 해당 O
- 사용자정의 함수 작성시 매개변수 작성시 아래 내용 해당 X

- 이유: 매개 변수 = register 변수
...
- 예: HEAP Overflow 표시: return n으로 Programme 종료, n = random int (e.g. 정수 0)
...
- 따라서 배열 요소값 전체를 전달 X + 포인터 변수 사용하여 주소"값만" 전달 O
- ➋ 배열 매개 변수 = 포인터 변수
...
- 예: 내림차순 정렬을 위한 MAX HEAP ALGORITHM 최대힙 알고리즘 구현
...
---
<!-- LESSON-NAV:START -->
[이전: 07. 메모리 컴파일 런타임](./07_메모리_컴파일_런타임.md) · [단원 목차](./README.md) · [다음: 09. 배열 반환](./09_배열_반환.md)
<!-- LESSON-NAV:END -->

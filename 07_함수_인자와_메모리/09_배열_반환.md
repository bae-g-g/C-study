...
> 이전 학습: [08. 메모리 구조와 관리](./08_메모리_구조와_관리.md)  
> 다음 학습: [10. 동적 메모리 반환](./10_동적_메모리_반환.md)

<p align="center">
  <img src="./img/mms5.png" alt="09. 배열 반환 대표 이미지" width="720">
</p>

### 학습 목표

- 배열 반환의 핵심 개념과 사용 목적을 설명할 수 있다.
- 예제 코드를 읽고 실행 흐름과 결과를 단계별로 추적할 수 있다.
- 이전 단원에서 배운 개념과 다음 단원에서 이어질 내용을 연결할 수 있다.

### 수업 흐름

1. ① Return Array 배열 반환
...
(위의 Compiling Error #1 + #2) + (큰변수 e.g. 배열, 구조체, 클라스에는 주소값으로 접근성 용이)
...
1. A 배열에서 붕괴된 포인터변수 반환 가능 X

- 예: return X; // int X\[3\] = {1, 2, 3}; : X ➝(붕괴 O)➝ &X\[0\]
...
- 예: return X\_ptr; // int X\[3\] = {1, 2, 3}; : int \*X\_ptr = &X\[0\];
...
- 이유: Stack Sector 메모리 "할당" 원리(쌓임) 특성

- 따라서 사용자정의 함수 Return (반환)한 포인터변수(주소값)으로 접근한 메모리 공간

≠ 배열 메모리 공간

의도한 초기화값 (e.g. 배열 요소 값)
...
- 따라서 Static 정적 포인터변수 or Dynamic Memory Allocation 동적 메모리 할당 이용
...
---
<!-- LESSON-NAV:START -->
[이전: 08. 메모리 구조와 관리](./08_메모리_구조와_관리.md) · [단원 목차](./README.md) · [다음: 10. 동적 메모리 반환](./10_동적_메모리_반환.md)
<!-- LESSON-NAV:END -->

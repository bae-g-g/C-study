...
> 이전 학습: [03. 함수 포인터 활용2](./03_함수_포인터_활용2.md)  
> 다음 학습: [05. 동기 비동기 블로킹 논블로킹 개념](./05_동기_비동기_블로킹_논블로킹_개념.md)

<p align="center">
  <img src="./img/F.png" alt="04. 함수 포인터 활용3 대표 이미지" width="720">
</p>

### 학습 목표

- 함수 포인터 활용3의 핵심 개념과 사용 목적을 설명할 수 있다.
...
- 예: HTML, CSS, JAVA SCRIPT = 텍스트 O, 프로그래밍 언어 X (∵ 컴파일 실행 X)
...
- 따라서 Case #1)의 결제 확인 과정 생략 가능 O

- 결제 처리

토서 서버 결제 처리: 쿠팡 서버 -------------- 토스 서버 간의 통신 X
...
| Case #1)의 결제 확인 과정 생략 가능 O<br>➝ 비용 절약<br><br>Cf)<br>시간 절약 측면은 명확 X<br>서버 간의 통신 X 시간 > Case #1)의 결제 확인 과정<br>의 확율도 무시 못함<br><br>Cf)<br>콜백 기능 단점 보완책 예시<br>10초 후 다시 실행 시간 소요 발생<br>Case #1)에서는 0.1초 후 바로 실행 | 토서 서버 문제 발생 시<br>쿠팡 서버 무한 대기<br>∴ 10초 후 다시 실행 요청 기능 필요 |
...
- 이유: 16번째줄에 함수 foo가 호출/실행되고, 09번째줄에 콜백 함수가 실행

(02번째줄 콜백함수의 주소값을 사용 하는 코드 내용 포함)

### ⓒ

### C언어(순차적 작동) 콜백 함수(동시 작동) 사용 빈번도 낮음
...
- 따라서 함수-사용자정의 함수/콜백 함수 실행 방식 (코드작성 법)의 차이
- 따라서 유연성 X
...
사용자정의 함수 호출 = 함수 주소값 호출 (함수명 ≒ 함수 주소값)
...
- 따라서 Modularity 모듈성 (Blockfication 블럭화) X
...
- 참고: 협업시 함수 integrate\_fun 이 Pre-Architected ➝ 개발자 A, B가 fun1, fun2를 작성/전달
...
- 참고: 여러 콜백 함수를 전달받는 함수 1개 필요

- 참고: 전달받는 함수 1개를 이용하여 여러 콜백 함수 호출/실행 작성 여러번 필요
...
- 참고: 해당 사용자정의 함수 호출/실행 작성 여러번 필요
...
- 예: Backward, Open Image, Off-Button, etc.
...
---
<!-- LESSON-NAV:START -->
[이전: 03. 함수 포인터 활용2](./03_함수_포인터_활용2.md) · [단원 목차](./README.md) · [다음: 05. 동기 비동기 블로킹 논블로킹 개념](./05_동기_비동기_블로킹_논블로킹_개념.md)
<!-- LESSON-NAV:END -->

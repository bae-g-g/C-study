...
> 이전 학습: [04. 함수 포인터 활용3](./04_함수_포인터_활용3.md)  
> 다음 학습: [06. 동기 블로킹](./06_동기_블로킹.md)

### 학습 목표

- 동기 비동기 블로킹 논블로킹 개념의 핵심 개념과 사용 목적을 설명할 수 있다.
...
4. ① KERNEL Definition 커널 정의
5. ② KERNEL Type 커널 유형
6. ③ KERNEL Programming 커널 프로그래밍
...
- ⓒ1 CPU는 실행에 필요한 DATA를 Register에 저장
...
- 따라서 커널 자체가 STACK을 가진다고 보기 어려움

- 커널은 OS 운영체제의 정체성을 결정

- 예: Linux KERNEL-Used 리눅스 커널 사용 ➝ Fedora 페도라, Ubuntu 우분투, etc.
...
.  멀틱스 운영체제(Made in MIT)에 기반한 다중 사용자용 서버운영 체제

.  개인용 데스크탑 or 임베디드용 운영체제 전신
...
. LINUX는 LINUX Foundation에서 관리 vs BSD는 관리자 없음
...
- 예: Input/Output Device Driver 입출력 장치 드라이버
...
- 예: TCP/IP Protocol Stack TCP/IP 프로토콜 스택
...
- 참고: Indeed, 어플리케이션이 커널에 제어권을 잠시 넘겨주고 바로 돌려받음|

- 참고: Indeed, 동기 + 논블로킹 = 동기 + 블로킹의 변형 = 동기 + 짧은 주기의 블로킹
...
---
<!-- LESSON-NAV:START -->
[이전: 04. 함수 포인터 활용3](./04_함수_포인터_활용3.md) · [단원 목차](./README.md) · [다음: 06. 동기 블로킹](./06_동기_블로킹.md)
<!-- LESSON-NAV:END -->
